...
func TestBuildExprASTExpr(t *testing.T) {
	tests := []struct {
		...
		{"intersect" /*	  */, "a1 x10:x9", "(INTERSECT (a1 RANGE (x10 x9)))"},
		...
		{"func" /*		  */, "IF(1,2,3)", "(FUNC IF (1 2 3))"},
		{"func" /*		  */, "IF(1,IF(2,3),IF(4,5))", "(FUNC IF (1 FUNC IF (2 3) FUNC IF (4 5)))"},
		{"func" /*		  */, "IF(1,sum(sum()))", "(FUNC IF (1 FUNC sum (FUNC sum)))"},
